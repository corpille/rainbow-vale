...
// these hold that boilerplate once so each shape below is just its style + its path
function fillStrokeGlow(fill, stroke, draw) {
  ctx.save();
...
// draw() owns its own beginPath()/stroke() calls (sometimes more than one subpath,
// e.g. an eye plus its lash) — this only holds the shared style + save/restore
function strokeShape(color, width, draw) {
  ctx.save();
...
function drawPonySide(moving, walkPhase) {
  ...
  fillShape('#ff9ad0', () => tracePath([[-14.5, -5.5]]));
}
